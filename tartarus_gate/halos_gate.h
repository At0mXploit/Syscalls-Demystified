...
// Tartarus Gate check added: verifies syscall (0F 05) or int 2E (CD 2E) at +0x12
// catches partially hooked stubs that preserve the prologue but corrupt the middle
...
static BOOLEAN GetSSNInternal( PBYTE NtFunction, PWORD SSN )
{
    ...
    do {
        ...
        {
            // Tartarus Gate: also verify the syscall instruction at +0x12
            // a partially hooked stub can preserve 4C 8B D1 B8 but corrupt the middle
            // valid stubs end with either syscall (0F 05) or int 2E (CD 2E)
